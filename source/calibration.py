...
def as_peaks_dataframe(f):
    def wrapper(*args):
        ...
        dict_of_dfs = {
            q: pd.DataFrame(nested_dict[q], index=index,).T.rename_axis(
                "channel"
            )
            for q in quadrants
        }
...
def as_cal_dataframe(f):
    def wrapper(*args):
        ...
        dict_of_dfs = {
            q: pd.DataFrame(nested_dict[q], index=CAL_PARAMS,).T.rename_axis(
                "channel"
            )
            for q in quadrants
        }
...
def as_slo_dataframe(f):
    def wrapper(*args):
        ...
        dict_of_dfs = {
            q: pd.DataFrame(nested_dict[q], index=LO_PARAMS,).T.rename_axis(
                "channel"
            )
            for q in quadrants
        }
...
class Calibration:
    ...
    def __call__(self, data):
        ...
        lost_events = len(data["ADC"]) - sum(
            (data["ADC"] < bins[-1]) & (data["ADC"] >= bins[0])
        )
        self._print(
            ":white_check_mark: Binned data. Lost {:.2f}% dataset.".format(
                100 * lost_events / len(data["ADC"])
            )
        )
        ...
        electron_evlist = make_electron_list(
            data, self.sdd_cal, self.sfit, self.couples, self.nthreads,
        )
    ...
    def _make_xhistograms(self, data):
        ...
        histograms = compute_histogram(
            value, data, bins, nthreads=self.nthreads,
        )
        return histograms
    ...
    def _detect_xpeaks(self, gain_guess, offset_guess):
        ...
        for quad in self.channels.keys():
            for ch in self.channels[quad]:
                ...
                try:
                    limits = find_xpeaks(
                        bins, counts, energies, gain_guess, offset_guess,
                    )
    ...
    def _fit_xradsources(self):
        ...
        for quad in self.xpeaks.keys():
            for ch in self.xpeaks[quad].index:
                ...
                counts = self.xhistograms.counts[quad][ch]
                limits = [(row[source]["lim_low"], row[source]["lim_high"])
        ...
        for quad in self.sdd_cal.keys():
            for ch in self.sdd_cal[quad].index:
                ...
                try:
                    ...
                    companion = self.companion(quad, ch)
                    if companion in self.sdd_cal[quad].index:
                        message = err.warn_peak_detection(quad, ch)
                    else:
                        # companion is off and counts is empty
                        message = err.warn_peak_detection_offcomp(quad, ch, companion)
                    logging.warning(message)
        ...
        for quad in self.sfit.keys():
            for ch in self.sfit[quad].index:
                if ch not in self.couples[quad].keys():
                    assert self.scintid(quad, ch) == self.companion(quad, ch)
                    continue
                scint = ch
                ...
                try:
                    limits = find_epeaks(
                        bins, counts, energies, lightout_guess,
                    )
                except err.DetectPeakError:
    ...
    def _scintillator_lout_error(self, quad, scint, energies):
        ...
        cell_cal = (
            self.sdd_cal[quad]
            .loc[scint][["gain", "gain_err", "offset", "offset_err",]]
            .to_list()
        )
        companion = self.couples[quad][cell]
        comp_cal = (
            self.sdd_cal[quad]
            .loc[companion][["gain", "gain_err", "offset", "offset_err",]]
            .to_list()
        )
    ...
    def _effective_lout_error(self, quad, scint, ch, companion):
        ...
        centers_companion = (
            self.sfit[quad].loc[companion][:, "center"].to_numpy()
        )
        ...
        errors = np.sqrt(
            ...
            + centers_err ** 2
            * (lo * centers_companion / (centers + centers_companion) ** 2)
            ** 2
...
def _fit_peaks(x, y, limits):
    ...
    for i in range(n_peaks):
        ...
        result, start, stop, x_fine, fitting_curve = _peak_fitter(
            x, y, (lowlim, hilim)
        )
