...
fitsfiles = ['output\\bigtable.fits']
...
for i, ASIC in enumerate(asics):
    ...
    if cleangamma:
        print(couples)
        counts_data = specutilities.clean_gamma(counts_data, couples, threshold=1100)
        ...
        for v in np.arange(32):
            if v in CHst:  # If the channel is in the list
                ...
                with sns.plotting_context("talk"):
                    fig1, ax = specutilities.linPlot('stretcher', ranges, v, xadc[v], xcalib_line, gain, offset, calib_units,
                                       figsize=(16, 6))
                    fig1.suptitle('QUADRANT ' + ASIC)
                    fig1.savefig(config.OUTDIR + '/Quad' + ASIC + '/' + ASIC + '_LINEARITY_CH' + str(v) + '.png')
                    plt.close(fig1)
